...
const LEARNING_PATHS_STORAGE_KEY = 'habit-learning-paths-v1';
...
let state = loadState();
let learningPaths = loadLearningPathsState();
let activePathHabitId = Object.keys(learningPaths.attachments).find((habitId) => state.habits.some((habit) => habit.id === habitId)) || null;
...
function emptyCurriculumState() { return { selectedPhase: 'diagnostic', checkpoints: {} }; }
function loadLearningPathsState() {
  ...
  if (state.habits.some((habit) => habit.id === TFM_HABIT_ID)) {
    ...
    try {
      const savedLegacy = JSON.parse(localStorage.getItem(CURRICULUM_STORAGE_KEY));
      if (TfmCurriculum.isValidCurriculumState(savedLegacy)) legacy = savedLegacy;
    } catch (error) { /* Ignore unreadable legacy progress. */ }
    learningPathState.attachments[TFM_HABIT_ID] = 'tfm-professional';
    learningPathState.curricula[TFM_HABIT_ID] = legacy;
  }
  return learningPathState;
}
function persistLearningPaths() { localStorage.setItem(LEARNING_PATHS_STORAGE_KEY, JSON.stringify(learningPaths)); }
function curriculumForHabit(habitId) { return learningPaths.curricula[habitId] || emptyCurriculumState(); }
function ensureCurriculumForHabit(habitId) { learningPaths.curricula[habitId] ||= emptyCurriculumState(); return learningPaths.curricula[habitId]; }
...
function renderPathPreview(habit) {
  ...
  const next = [
    ['learn', 'Explain the core ideas from memory'],
    ['build', 'Create the phase’s reproducible artifact'],
    ['defend', 'Pass the gate and record the evidence']
  ].find(([key]) => !checkpoints[key]);
  const nextStep = next?.[1] || 'Review the evidence and advance';
  return `<button class="path-preview" type="button" data-action="open-path" data-id="${habit.id}"><span><b>Learning path</b> ${phase.number} · ${phase.title}</span> <small>Next · ${nextStep}</small></button>`;
}
function renderHeatmap() {
  ...
  const cells = ['<div></div>', ...Array.from({ length: 53 }, (_, week) => `<span class="month-label" style="grid-column:${week + 2};grid-row:1">${monthStarts[week] || ''}</span>`), ...['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((label, index) => `<span class="weekday-label" style="grid-column:1;grid-row:${index + 2}">${label}</span>`), ...days.map((date) => {
    ...
    const detail = `${key}: ${formatCount(count, 'habit')} complete${selectedHabit ? `; toggle ${selectedHabit.name}` : ''}`;
    return `<button class="heat-cell level-${Math.min(count, 3)} ${future ? 'future' : ''}" style="grid-column:${Math.floor(days.indexOf(date) / 7) + 2};grid-row:${(date.getDay() || 7) + 1}" type="button" data-date="${key}" title="${escapeHtml(detail)}" aria-label="${escapeHtml(detail)}" ${future ? 'disabled' : ''}></button>`;
  })];
...
function renderCurriculum() {
  ...
  const selected = TfmCurriculum.PHASES.find((phase) => phase.id === curriculum.selectedPhase) || recommended;
  const progress = TfmCurriculum.curriculumProgress(curriculum);
  const checkpoints = curriculum.checkpoints[selected.id] || {};
  ...
  els.phaseRail.innerHTML = TfmCurriculum.PHASES.map((phase) => {
    const complete = TfmCurriculum.CHECKPOINTS.every((key) => curriculum.checkpoints[phase.id]?.[key]);
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'focus') {
    ...
    render();
    showToast(`${state.habits.find((habit) => habit.id === selectedHabitId).name} selected for heatmap logging`);
  }
  if (action === 'open-path') {
    activePathHabitId = control.dataset.id;
    renderCurriculum();
    els.pathDialog.showModal();
    document.querySelector('#close-path-dialog').focus();
  }
  if (action === 'select-phase') {
    ensureCurriculumForHabit(activePathHabitId).selectedPhase = control.dataset.phase;
    persistLearningPaths();
    renderCurriculum();
  }
  if (action === 'toggle-checkpoint') {
    const curriculum = ensureCurriculumForHabit(activePathHabitId);
    const phase = control.dataset.phase;
    const checkpoint = control.dataset.checkpoint;
    curriculum.checkpoints[phase] ||= {};
    curriculum.checkpoints[phase][checkpoint] = !curriculum.checkpoints[phase][checkpoint];
    persistLearningPaths();
    renderCurriculum();
    renderHabits();
  ...
  if (action === 'toggle-path-habit') {
    ...
    showToast(isComplete(habit) ? 'Study day logged' : 'Study log undone');
  }
  if (action === 'edit') openDialog(state.habits.find((habit) => habit.id === control.dataset.id));
  if (action === 'delete') {
    const habit = state.habits.find((item) => item.id === control.dataset.id);
    if (!habit || !confirm(`Delete "${habit.name}" and its history?`)) return;
  ...
  if (event.target.matches('.heat-cell:not(.future)')) {
    const date = event.target.dataset.date;
    const selectedHabit = state.habits.find((habit) => habit.id === selectedHabitId) || state.habits[0];
    if (selectedHabit) {
      toggleCompletion(selectedHabit.id, date);
      showToast(`${selectedHabit.name} updated for ${date}`);
    }
  }
});
...
els.form.addEventListener('submit', (event) => {
  ...
  if (habit) {
    habit.name = name;
    habit.color = els.color.value;
    showToast('Habit updated');
  } else {
    habit = { id: crypto.randomUUID(), name, color: els.color.value, completions: {} };
